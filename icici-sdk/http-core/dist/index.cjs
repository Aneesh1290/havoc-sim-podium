...
dotenv$1.config({ path: path.join(rootDir, ".env") });
const dirname$1$1 = directory$1();
const logDirectory$1 = path.join(dirname$1$1, "..", "..", "logs");
const logFile$1 = path.join(logDirectory$1, "sdk.log");
...
class UtilsService {
    ...
    static async readBulkRequestFile(filePath) {
        try {
            ...
            await fs.promises.access(absolutePath).catch(() => {
...
class DTOService {
    ...
    static validateDTO(payload) {
        const errors = classValidator.validateSync(payload);
        if (errors.length > 0) {
